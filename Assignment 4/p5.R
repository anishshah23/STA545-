...
# specifying the loop for finding a cv error 
...
{   
    ...
    for(j in 1:k)
    {
	  ...
	  train.data<- spam[data.sample,]
        ...
        network <- neuralnet(formulae,data=train.data,hidden=hidden_1,err.fct='ce', linear.output=FALSE,
	  threshold=0.5)
	  ...
	  predict <- round(predict.network$net.result)
        ...
        # the cv error can be defined as folows
...
# 1 - selecting the hidden neural networks and definnig vectors
error_train <- NULL
error_test <- NULL
set.seed(100)
for(i in 1:5)
...
#plot type 'l4' will be truncated to first character
...
data.sample <- sample(1:nrow(spam),round(nrow(spam)/2))
	  train.data<- spam[data.sample,]
        test.data <- spam[-data.sample,]     
network <- neuralnet(formulae,data=train.data,hidden=5,err.fct='ce', linear.output=FALSE,
	  threshold=0.5)
